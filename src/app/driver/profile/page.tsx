...
function StarIcon({ filled }: { filled: boolean }) {
  return (
    <svg className={`w-4 h-4 ${filled ? "text-warning-700 dark:text-warning-500" : "text-surface-200"}`} viewBox="0 0 24 24" fill="currentColor">
          ...
          {editingPhone ? (
            <div className="flex items-center gap-2 min-w-0">
              <input
                ...
                className="text-sm font-medium text-surface-900 bg-surface-50 dark:bg-surface-900 border border-border rounded-md px-2 py-1 w-36 focus:outline-none focus:ring-2 focus:ring-brand-700"
                autoFocus
              />
              <button
                onClick={savePhone}
