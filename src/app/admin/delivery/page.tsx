...
const COLUMNS: { status: DispatchStatus; label: string; bgClass: string; countClass: string }[] = [
  ...
  { status: "failed",    label: "Failed",             bgClass: "bg-danger-50 dark:bg-danger-500/10",    countClass: "bg-danger-100 dark:bg-danger-500/20 text-danger-600 dark:text-foreground" },
];
  ...
  return (
    <Card className="p-3.5 space-y-2.5 hover:shadow-card-md transition-shadow">
      ...
      {dispatchStatus === "failed" && failReason && (
        <div className="flex items-center gap-1.5 rounded-lg bg-danger-50 dark:bg-danger-500/20 border border-danger-200 dark:border-danger-500/30 px-2 py-1.5 text-xs text-danger-600 dark:text-foreground">
      ...
      {dispatchStatus === "out" && (
        <div className="grid grid-cols-2 gap-1.5">
          ...
          <Button
            size="sm"
            variant="outline"
            className="text-xs h-8 text-danger-600 dark:text-danger-500 border-danger-200 dark:border-danger-500/40 hover:bg-danger-50 dark:hover:bg-danger-500/10"
            onClick={() => onFailed(id)}
          >
    ...
    <div className="p-6 space-y-5 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: "Failed", value: counts.failed, icon: XCircle, color: "bg-danger-50 dark:bg-danger-500/10 text-danger-600 dark:text-foreground", border: "border-danger-200 dark:border-danger-500/20" },
            ...
            <div className="space-y-2">
              {FAIL_REASONS.map((reason) => (
                <button key={reason} onClick={() => setFailReason(reason)}
                  className={`w-full flex items-center gap-3 rounded-xl border px-4 py-3 text-sm font-medium text-left transition-colors ${failReason === reason ? "border-danger-400 dark:border-danger-500/50 bg-danger-50 dark:bg-danger-500/20 text-danger-600 dark:text-foreground" : "border-border bg-background text-foreground"}`}>
