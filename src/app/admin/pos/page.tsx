    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-success-50 p-6">
        <div className="bg-card rounded-3xl shadow-card-md p-8 w-full max-w-md space-y-6">
          ...
          <button
            onClick={reset}
            className="w-full h-12 rounded-2xl bg-brand-500 text-white font-bold text-base hover:bg-brand-600 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
          >
        ...
        <div className="flex md:hidden items-center border border-border rounded-xl overflow-hidden text-[11px]">
          {(["products", "cart", "pay"] as const).map(t => (
            <button
              ...
              className={cn("relative px-3 py-2 font-medium capitalize transition-colors", mobileTab === t ? "bg-brand-500 text-white" : "text-muted-foreground")}
            >
              {t === "cart" && cartCount > 0 && <span className="absolute top-0.5 right-0.5 h-3.5 w-3.5 rounded-full bg-danger-500 text-white text-[8px] flex items-center justify-center">{cartCount > 9 ? "9+" : cartCount}</span>}
        ...
        <div className={cn("flex flex-col flex-1 min-w-0 overflow-hidden print-hide", mobileTab !== "products" && "hidden md:flex")}>
          <div className="px-3 pt-3 pb-2 border-b border-border space-y-2 shrink-0">
            ...
            <div className="flex gap-1.5 overflow-x-auto scrollbar-hide">
              ...
              {CATEGORIES.map(c => (
                <button key={c.id} onClick={() => setCategory(c.id)} className={cn("shrink-0 rounded-lg px-3 py-1.5 text-[11px] font-semibold border transition-colors", category === c.id ? "bg-brand-500 text-white border-brand-500" : "bg-card border-border text-muted-foreground")}>{c.name}</button>
              ))}
            </div>
          </div>
                ...
                return (
                  <button key={product.id} onClick={() => addItem(product)} disabled={outOfStock}
                    ...
                    <div className={cn("h-20 flex items-center justify-center relative bg-gradient-to-br", d.gradient)}>
                      ...
                      {inCart && <span className="absolute top-1.5 right-1.5 h-6 w-6 rounded-full bg-brand-500 text-white text-[11px] font-bold flex items-center justify-center shadow">{inCart.quantity}</span>}
            ...
            <div className="p-3 space-y-3">
              ...
              <button
                ...
                className={cn(
                  ...
                  canCharge && !loading
                    ? "bg-brand-500 text-white hover:bg-brand-600 active:scale-[0.98] shadow-lg"
