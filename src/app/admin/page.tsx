...
export default function AdminDashboardPage() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: TrendingUp,
      href: "/admin/reports",
      color: "text-success-700 dark:text-foreground bg-success-50 dark:bg-success-500/10",
    },
    {
      ...
      color: "text-brand-700 dark:text-foreground bg-brand-50 dark:bg-brand-500/10",
    },
    {
      ...
      color: "text-warning-700 dark:text-foreground bg-warning-50 dark:bg-warning-500/10",
    },
  ];
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
              ...
              <Card className="hover:shadow-card-md transition-shadow cursor-pointer">
                <CardContent className="p-5">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className={`flex items-center gap-1 text-xs font-medium ${s.up ? "text-success-700 dark:text-foreground" : "text-warning-700 dark:text-foreground"}`}>
                      {s.up ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                    </div>
                  </div>
                  <p className="font-display text-2xl font-bold text-foreground">{s.value}</p>
                  <p className="text-xs text-muted-foreground mt-1">{s.label}</p>
                  <p className={`text-xs mt-0.5 font-medium ${s.up ? "text-success-700 dark:text-foreground" : "text-warning-700 dark:text-foreground"}`}>{s.change}</p>
                </CardContent>
              </Card>
            </Link>
          ))
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {[
              ...
              { label: "Out for delivery", count: stats.outForDelivery, color: "text-brand-700 dark:text-foreground bg-brand-50 dark:bg-brand-500/10", href: "/admin/delivery" },
      ...
      {usingFallback && !loading && (
        <div className="rounded-xl border border-warning-200 bg-warning-50 dark:bg-warning-500/10 px-4 py-2.5 text-sm text-warning-700 dark:text-foreground flex items-center gap-2">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Link href="/admin/orders" className="flex items-center gap-1 text-sm text-brand-700 dark:text-brand-400 hover:text-brand-700">
                  ...
                  recentOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-muted/50 transition-colors">
                      <td className="px-5 py-3.5">
                        <Link href={`/admin/orders/${order.id}`} className="font-medium text-foreground hover:text-brand-700">
                          {order.orderNumber}
                        </Link>
                      </td>
